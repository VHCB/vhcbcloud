...
@Rent bit, @Mortgage bit, @Utilities bit, @HHincludes nvarchar(10)
...
select Primary_ASO_code as ProjectId, UUID, 
	...
	isnull(Rent, 0) as Rent, isnull(Mortg, 0) as Mortgage, isnull(Util, 0) as Utilities, Includes as HHincludes
from [dbo].[MasterProj_2] mp
join dbo.HOPWA_STRMU_8 ra8(nolock) on mp.ProjectId = ra8.Primary_ASO_code
	...
	select @HOPWAID, 26777, 201, @livingsituationID, @Yr1, @Yr2, @Yr3, @StartDate, null, null
	...
	--HOPWAExp
	insert into vhcb.dbo.HOPWAExp(HOPWAProgramID, Fund, TransType, Amount, Rent, Mortgage, Utilities)
	select @HOPWAProgramID, 201, null, 0,  @Rent, @Mortgage, @Utilities
