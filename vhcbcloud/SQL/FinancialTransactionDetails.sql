...
begin
	...
	join lookupvalues lpn on lpn.typeid = pn.lkprojectname
	join trans tr on tr.projectid = p.projectid
	where defname = 1 and tr.LkTransaction = 236-- and tr.lkstatus != 261 
	and tr.RowIsActive=1 and pn.defname=1	and p.Proj_num like @filter +'%'	
	order by proj_num 
end
go
...
Begin
	select p.projectid, det.FundId, det.lktranstype, ttv.description as FundType,
				f.name, f.account, p.proj_num, lv.Description as projectname, f.abbrv,				
				case 
					when tr.lkstatus = 261 then 'Pending'
					when tr.lkstatus = 262 then 'Final'
				 end as lkStatus, 
		...
		and  f.RowIsActive=1 and p.ProjectId = @projectid and f.fundid = @fundId and det.lktranstype = @transtype
		and tr.RowIsActive=1 and pn.DefName =1 and det.rowisactive = 1 and tr.lkstatus = 261
		group by  det.FundId, det.LkTransType ,  p.ProjectId, p.Proj_num, lv.Description,   f.name, 
		f.abbrv, tr.lkstatus, ttv.description, f.account
		order by f.name
...
begin
	...
	declare @tempFundCommit table (
		...
		End
			...
			select projectid,(sum(isnull(commitmentamount,0))- ( sum(isnull (pendingdisburse,0))+ sum(isnull(finaldisbursedamount,0)))) as availFund from @tempAvailFunds
			group by projectid
