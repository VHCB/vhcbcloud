...
select Projectid, OtherNames, PrimaryProduct,  CONVERT (VARCHAR(50), YrManageBus, 50)  from FinalViabConversionApplicant_v
...
select Projectid, 26236, getdate() from FinalViabConversionApplicant_v where Organic != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26237, getdate() from FinalViabConversionApplicant_v where Conserved != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26238, getdate() from FinalViabConversionApplicant_v where Transfer != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26239, getdate() from FinalViabConversionApplicant_v where OnFarmProc != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26448, getdate() from FinalViabConversionApplicant_v where LCB != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26677, getdate() from FinalViabConversionApplicant_v where OutOFBiz != 0
...
select ProjectID, [AcresInProduction], AcresOwned,[AcresLeased], [Acres from Access] from  FinalViabConversionApplicant_v
go
