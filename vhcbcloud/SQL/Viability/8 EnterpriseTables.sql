...
select Projectid, OtherNames, PrimaryProduct,  CONVERT (VARCHAR(50), YrManageBus, 50)  from AdditionalProjectsFarmDups_v
...
select Projectid, 26236, getdate() from AdditionalProjectsFarmDups_v where Organic != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26237, getdate() from AdditionalProjectsFarmDups_v where Conserved != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26238, getdate() from AdditionalProjectsFarmDups_v where Transfer != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26239, getdate() from AdditionalProjectsFarmDups_v where OnFarmProc != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26448, getdate() from AdditionalProjectsFarmDups_v where LCB != 0

insert into EnterpriseAttributes(ProjectID, LKAttributeID, Date)
select Projectid, 26677, getdate() from AdditionalProjectsFarmDups_v where OutOFBiz != 0
...
select ProjectID, [AcresInProduction], AcresOwned,[AcresLeased], isnull([Acres from Access], 0) from  AdditionalProjectsFarmDups_v
go
