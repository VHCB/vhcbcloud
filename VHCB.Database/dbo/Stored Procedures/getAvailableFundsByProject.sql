...
as
begin
--exec getAvailableFundsByProject 6597
	...
	begin
		...
		End
			...
			select projectid, (sum(isnull(commitmentamount,0))- (sum(abs(isnull (pendingdisburse, 0))) + sum(isnull(finaldisbursedamount,0)))) as availFund 
			from @tempAvailFunds
			group by projectid
