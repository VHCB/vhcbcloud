...
Begin
	...
	if (@isReallocation=1)
	Begin
	insert into @tempFundCommit (transID, detailid, projectid, fundid, account, lktranstype, FundType, FundName, Projnum, ProjectName, ProjectCheckReqID, 
	...
	f.abbrv, tr.lkstatus, ttv.description, f.account, tr.transid, det.DetailID, det.Amount
	order by p.Proj_num
	end
	else
	Begin
	insert into @tempFundCommit (transid, detailid, projectid, fundid, account, lktranstype, FundType, FundName, Projnum, ProjectName, ProjectCheckReqID, 
		...
		left join LkTransType_v ttv(nolock) on det.lktranstype = ttv.typeid
		where tr.LkTransaction in (238,239,240,26552) and tr.ProjectID = @projectid and
		tr.RowIsActive=1 and pn.DefName =1 and det.rowisactive = 1 and tr.Balanced = 1
		group by det.FundId, det.LkTransType ,  p.ProjectId, p.Proj_num, lv.Description, ProjectCheckReqID, f.name, 
		f.abbrv, tr.lkstatus, ttv.description, tr.transid, det.DetailID, f.account
		order by p.Proj_num
	End
		...
		select  tr.transid, det.detailid, p.projectid, det.FundId, f.account, det.lktranstype, 
		...
		and tr.RowIsActive=1 and pn.DefName =1 and det.rowisactive = 1 and p.ProjectId = @projectid and tr.Balanced = 1
		group by det.FundId, det.LkTransType ,  p.ProjectId, p.Proj_num, lv.Description, tr.ProjectCheckReqID, f.name,
		f.abbrv, tr.lkstatus, ttv.description, tr.transid, det.DetailID, f.account
		order by p.Proj_num
	
	select transid, detailid, projectid, fundid, account, lktranstype, FundType, FundName, Projnum, ProjectName, FundAbbrv, 
			   ...
			   max(Date) as [date]
	from @tempFundCommit
	group by projectid, transid, detailid, fundid,account, lktranstype,FundType, FundName, FundAbbrv, Projnum, ProjectName


		select p.projectid, tr.transid, det.detailid,
		...
		and tr.RowIsActive=1 and det.RowIsActive=1 and p.projectid = @projectid and tr.Balanced = 1
		order by p.Proj_num
